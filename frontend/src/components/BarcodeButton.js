 ...
 */
function BarcodeButton({ kind, id, code, label = "", subtitle = "", onAssigned }) {
  ...
  const openPreview = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Could not build the barcode");
      setOpen(false);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <button
        ...
        onClick={(event) => { event.stopPropagation(); openPreview(); }}
      ...
      {open && (
        <Modal onClose={() => setOpen(false)} title={`Barcode — ${label || barcode || ""}`}>
          <div style={{ textAlign: "center", padding: "8px 4px" }}>
            {subtitle && (
              <p style={{ color: "#64748b", fontSize: "0.85rem", margin: "0 0 10px" }}>{subtitle}</p>
            )}
            {loading ? (
              <p>Generating…</p>
            ) : (
              <>
                <div
                  style={{ background: "#fff", padding: 10, borderRadius: 6, display: "inline-block", maxWidth: "100%", overflowX: "auto" }}
                  // The SVG comes from our own API and contains no scripts.
                  dangerouslySetInnerHTML={{ __html: svg }}
                />
                <p style={{ fontFamily: "monospace", letterSpacing: 1, marginTop: 10 }}>{barcode}</p>
