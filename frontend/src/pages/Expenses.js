...
const Expenses = () => {
  ...
  const {
    expenses: ctxExpenses, categories, employees, stats,
  ...
  const fetchExpenses = useCallback(async () => {
    try {
      setLoading(true);
      const params = { page, limit, ...(search && { search }), ...(statusFilter && { status: statusFilter }), ...(categoryFilter && { category: categoryFilter }) };
  ...
  }, [page, limit, search, statusFilter, categoryFilter, loadExpenses, setExpenses]);

  useEffect(() => { if (currentUser) fetchExpenses(); }, [currentUser, fetchExpenses]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { setPage(1); fetchExpenses(); }, 300);
    return () => clearTimeout(timer);
  }, [currentUser, search, statusFilter, categoryFilter, fetchExpenses]);

  const openExpModal = (mode, exp = null) => {
    setExpModalMode(mode); setSelectedExpense(exp); setShowExpModal(true);
  };
  const closeExpModal = () => { setShowExpModal(false); setSelectedExpense(null); };

  const openCatModal = (mode, cat = null) => {
    setCatModalMode(mode); setSelectedCategory(cat); setShowCatModal(true);
  };
  const closeCatModal = () => { setShowCatModal(false); setSelectedCategory(null); };

  const handleCreateExpense = async (formData) => {
    const result = await createExpense(formData);
    if (result.success) { closeExpModal(); fetchExpenses(); }
  ...
  const handleDeleteExpense = async () => {
    ...
    if (result.success) { setConfirmDelete(null); fetchExpenses(); }
    else if (result.error) setErrorPopup(result.error);
  };
  ...
  const handleCreateCategory = async (formData) => {
    ...
    if (result.success) { closeCatModal(); }
    else if (result.error) setErrorPopup(result.error);
  };
    ...
    <div className="user-management-page">
      <div className="page-header">
        <div className="header-content">
          <h1>Expenses</h1>
          <p className="subtitle">Workshop, general, and salary-linked expenses</p>
        </div>
        <div className="header-actions">
          <button className={`btn ${tab === 'expenses' ? 'btn-primary' : 'btn-secondary'}`}
            onClick={() => setTab('expenses')}>Expenses</button>
          <button className={`btn ${tab === 'categories' ? 'btn-primary' : 'btn-secondary'}`}
            onClick={() => setTab('categories')}>Categories</button>
          {tab === 'expenses' && (
            <button className="btn btn-primary btn-create" onClick={() => openExpModal('create')}>
              <span className="icon">+</span> New Expense
            </button>
          )}
          {tab === 'categories' && (
            <button className="btn btn-primary btn-create" onClick={() => openCatModal('create')}>
        ...
        <>
          <div className="filters-bar">
            <div className="search-box">
              <input type="text" placeholder="Search by description, vendor..."
          ...
          <div className="table-container desktop-only">
            {loading ? (
              <div className="loading-state"><div className="spinner"></div><p>Loading expenses...</p></div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {ctxExpenses.map(exp => {
                    ...
                    return (
                      <tr key={id} className={isPosted ? 'row-inactive' : ''}
                        onClick={() => setDrawerExpense(exp)} style={{ cursor: 'pointer' }}>
                        ...
                        <td>{exp.amount != null ? Number(exp.amount).toLocaleString() : '-'}</td>
                        <td>{exp.vendor || '-'}</td>
                        <td>
                          <span className={`badge ${exp.status === 'posted' ? 'badge-success' : 'badge-secondary'}`}>
                            {exp.status || '-'}
                          </span>
                        </td>
                        <td onClick={e => e.stopPropagation()}>
                          <div className="action-buttons">
                            ...
                            {canPost && !isPosted && exp.status !== 'draft' && (
                              <button className="btn btn-sm btn-primary" onClick={() => handlePostExpense(id)}>Post</button>
                            )}
                            {isPosted && <span className="badge badge-success">Posted</span>}
                          </div>
                        </td>
                      </tr>
                    );
          ...
          <div className="mobile-only">
            {loading ? (
              ...
              <div className="empty-state"><div className="empty-icon">💸</div><h3>No Expenses Found</h3></div>
            ) : (
              <div className="cards-grid">
                {ctxExpenses.map(exp => {
                  ...
                  return (
                    <div key={id} className="data-card" onClick={() => setDrawerExpense(exp)}>
                      <div className="data-card-top">
                        <div className="data-card-avatar">
                          💸
                        </div>
                        <div className="data-card-info">
                          <span className="data-card-title">{exp.expenseNumber || '-'}</span>
                          <span className="data-card-subtitle">{exp.vendor || '-'}</span>
                        </div>
                        <span className={`badge-pill status-${s}`}>{exp.status || '-'}</span>
                      </div>
                      <div className="data-card-body">
                        <div className="data-card-row">
                          <span className="row-icon">📅</span>
                          <span className="row-label">Date</span>
                          <span className="row-value">{exp.expenseDate ? new Date(exp.expenseDate).toLocaleDateString('en-GB') : '-'}</span>
                        </div>
                        <div className="data-card-row">
                          <span className="row-icon">💰</span>
                          <span className="row-label">Amount</span>
                          <span className="row-value">{exp.amount != null ? Number(exp.amount).toLocaleString() : '-'}</span>
                        </div>
                        <div className="data-card-row">
                          <span className="row-icon">🏷</span>
                          <span className="row-label">Category</span>
                          <span className="row-value">{exp.category || '-'}</span>
                        </div>
                      </div>
                      <div className="data-card-footer" onClick={e => e.stopPropagation()}>
                        {canPost && s !== 'posted' && s !== 'draft' && (
                          <button className="btn btn-sm btn-primary" onClick={() => handlePostExpense(id)} style={{ marginRight: 'auto' }}>Post</button>
                        )}
                        {s !== 'posted' && (
                          <ActionButtons
                            onEdit={() => openExpModal('edit', exp)}
          ...
          <ExpenseFormModal
            isOpen={showExpModal} mode={expModalMode} initialData={selectedExpense}
            categories={categories} employees={employees}
            onClose={closeExpModal}
            onSubmit={expModalMode === 'create' ? handleCreateExpense : handleUpdateExpense}
            loading={saving}
          />
        </>
      )}
          ...
          <table className="data-table">
            ...
            <tbody>
              {categories.map(cat => {
                ...
                return (
                  <tr key={id} onClick={() => setDrawerCategory(cat)} style={{ cursor: 'pointer' }}>
                    <td>{cat.name || '-'}</td>
                    <td><code>{cat.code || '-'}</code></td>
                    <td><span className="badge badge-info">{GROUP_LABELS[cat.categoryGroup] || cat.categoryGroup || '-'}</span></td>
                    <td>
      ...
      <CategoryFormModal
        isOpen={showCatModal} mode={catModalMode} initialData={selectedCategory}
        onClose={closeCatModal}
        onSubmit={catModalMode === 'create' ? handleCreateCategory : handleUpdateCategory}
        loading={saving}
      />

      <ExpenseDrawer isOpen={!!drawerExpense} onClose={() => setDrawerExpense(null)} expense={drawerExpense} />
      <CategoryDrawer isOpen={!!drawerCategory} onClose={() => setDrawerCategory(null)} category={drawerCategory} />
    </div>
  );
};
